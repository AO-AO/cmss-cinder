...
class InvalidInstanceStatus(Invalid):
    message = _("Invalid instance status: %(reason)s")
...
class QemuGANotEnable(CinderException):
    message = _("QEMU guest agent is not enabled, image property"
                " has no hw_qemu_guest_agent")
    code = 409
    safe = True


class QemuGANotAvailable(CinderException):
    message = _("Guest agent not available for now, maybe QEMU-GA"
                " was down or user killed ")
    code = 409
    safe = True


class QemuGARepeatFreeze(CinderException):
    message = _("Domain is already quiesced.")
    code = 409
    safe = True


class QemuGARepeatThaw(CinderException):
    message = _("Domain is not quiesced.")
    code = 409
    safe = True


class NotPermitVmState(CinderException):
    message = _("Desired state must be active, paused, suspended,"
                " shelved_offloaded")
    code = 400
    safe = True


class ExecCmdError(CinderException):
    message = _("Run cmd error")
    code = 409
    safe = True


class DetachVolumeError(CinderException):
    message = _("detach volume error: %(reason)s")
    code = 409
    safe = True


class AttachVolumeError(CinderException):
    message = _("attach volume error: %(reason)s")
    code = 409
    safe = True
...
class BackupRestoreCannotDetach(CinderException):
    message = _("Cannot detach volume from VM")


class BackupRestoreCannotAttach(CinderException):
    message = _("Cannot attach volume to VM")
