...
class VolumeActionsController(wsgi.Controller):
    ...
    def _set_bootable(self, req, id, body):
        ...
        if bootable and volume['multiattach']:
            msg = _("Multiattach volume cannot set as bootable")
    ...
    def _revert(self, req, id, body):
        ...
        if isinstance(force, basestring):
            try:
                ...
                msg = _("Bad value for 'force'")
                raise webob.exc.HTTPBadRequest(explanation=msg)
        elif not isinstance(force, bool):
            msg = _("'force' not string or bool")
            raise webob.exc.HTTPBadRequest(explanation=msg)

        if not force and volume_status != 'available':
            msg = _("Volume is not in available status,"
                    "please revert by force")
            raise webob.exc.HTTPConflict(explanation=msg)
        elif force and volume_status not in force_valid_status:
            msg = _("Volume must be in available, error or in-use status")
            raise webob.exc.HTTPConflict(explanation=msg)
        ...
        self.volume_api.revert(context, volume, snapshot['id'])
        return webob.Response(status_int=202)
