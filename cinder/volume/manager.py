...
class VolumeManager(manager.SchedulerDependentManager):
    ...
    def attach_volume(self, context, volume_id, instance_uuid, host_name,
        ...
        def do_attach():
            ...
            if (volume['status'] == 'in-use' and not
                    volume['multiattach'] and not
            ...
            info = {'instance_id': instance_uuid}
            ...
            LOG.debug("Start to notify attach.end with info %s", info)
            self._notify_about_volume_usage(context, volume,
                                            "attach.end",
                                            extra_usage_info=info)
    ...
    def detach_volume(self, context, volume_id, attachment_id=None):
        ...
        volume = self.db.volume_get(context, volume_id)
        info = {'instance_id': attachment.get('instance_uuid')}
        self._notify_about_volume_usage(context, volume,
                                        "detach.start",
                                        extra_usage_info=info)
        try:
        ...
        LOG.debug("Start to notify detach.end with info %s", info)
        self._notify_about_volume_usage(context, volume,
                                        "detach.end",
                                        extra_usage_info=info)
    ...
    def _clone_image_volume(self, ctx, volume, image_meta):
        ...
        try:
            self.create_volume(ctx, image_volume.id,
                               allow_reschedule=False, source_volid=volume.id)
        ...
        try:
            ...
            if (self.driver.configuration.image_upload_use_cinder_backend
                ...
                LOG.debug("Registered image volume location to glance "
                          "image-id: %(image_id)s.",
                          {'image_id': image_meta['id']}, resource=volume)
            else:
    ...
    def revert(self, context, volume_id, snapshot_id):
        @utils.synchronized("%s-%s" % (snapshot_id, 'delete_snapshot'),
                            external=True)
        def do_revert():
            volume = self.db.volume_get(context, volume_id)
            snapshot = self.db.snapshot_get(context, snapshot_id)
            stat_record = volume.status
            size_record = snapshot.volume_size

            self._notify_about_snapshot_usage(context, snapshot,
                                              'revert.start')

            updates = {'status': 'reverting'}
            self.db.volume_update(context, volume_id, updates)

            try:
                LOG.info("Revert volume %(volume_id)s to "
                         "snapshot %(snapshot_id)s.",
                         {'volume_id': volume_id, 'snapshot_id': snapshot_id})
                self.driver.revert_to_snapshot(volume, snapshot)
            except Exception as e:
                with excutils.save_and_reraise_exception():
                    LOG.error("Revert fail %(volume_id)s: %(error)s",
                              {'volume_id': volume_id, 'error': e})
                    updates = {'status': 'error_reverting'}
                    self.db.volume_update(context, volume_id, updates)

            if stat_record == 'error':
                updates = {'status': 'available', 'size': size_record}
            else:
                updates = {'status': stat_record, 'size': size_record}
            self.db.volume_update(context, volume_id, updates)
            LOG.info("Revert successfully, volume is '%(status)s' "
                     "and '%(size)sG' now.",
                     {'status': updates['status'], 'size': updates['size']})
            self._notify_about_snapshot_usage(context, snapshot,
                                              'revert.end')
