...
##                                                                                                ##
##  Note      :                                                                                   ##
##  $Date:: 2015-06-28 16:54:25 +0900#$                                                           ##
##  $Revision: 10848 $                                                                            ##
##  File Name : image_util.py                                                                     ##
...
def _convert_image(prefix, source, dest, out_format, size, run_as_root=True):
    """Convert image to other format."""
    def _write_image(source, dest, start=0, count=None):
        """Write image to volume"""
        cmd = ('dd','if=%s' % source, 'of=%s' % dest, 'bs=1M', 'skip=%s' % start, 'seek=%s' % start)

        if count is not None:
            cmd += ('count=%s' % count, 'conv=sparse')
        else:
            cmd += ('conv=sparse,fsync',)

        try:
            (out, msg)=utils.execute(*cmd, run_as_root=True)
        except processutils.ProcessExecutionError as ex:
            msg=(_('_write_image Error, %s' % str(cmd)))
            LOG.error(msg)
            LOG.error(ex.stderr)
    ...
    # convert glance image to raw image
    tmpdest = source + "_fjrawtmp"
    conv_cmd = prefix + ('qemu-img', 'convert',
                         '-O', out_format, source, tmpdest)
    utils.execute(*conv_cmd,   run_as_root=True)

    # write image parallely (10 job) + finilizing job
    count = int(size/10485760) #image real size /(1M*10)
    try:
        ths = []
        for i in range(10):
            th = threading.Thread(target=_write_image, args=(tmpdest, dest), 
                                  kwargs={'count':'%s' % count, 'start':'%s' % str(i*count)})
            ths.append(th)
            th.start()
        # end of for range(10)
        ...
        #_write_image(tmpdest, dest, start=10*count)
        _write_image(tmpdest, dest, start=10*count)
    
    finally:   
        # remove temporary file
        remove_cmd =  ('rm', '-f', tmpdest)
        utils.execute(*remove_cmd, run_as_root=True)
...
def convert_image(source, dest, out_format, size, run_as_root=True, throttle=None):
    if not throttle:
...
def fetch_to_volume_format(context, image_service,
    ...
    with temporary_file() as tmp:
        ...
        convert_image(tmp, dest, volume_format, data.virtual_size,
                      run_as_root=run_as_root)
...
def upload_volume(context, image_service, image_meta, volume_path,
    ...
    with temporary_file() as tmp:
        ...
        convert_image(volume_path, tmp, image_meta['disk_format'], data.virtual_size,
                      run_as_root=run_as_root)
