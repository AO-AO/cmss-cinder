...
class SheepdogClient(object):
    ...
    def get_addr(self, type='management'):
        """Get a random node in sheepdog cluster."""
        if type=='external':
            return self.external_node_list[random.randint(0, len(self.external_node_list) - 1)]
        if type=='internal':
            return self.internal_node_list[random.randint(0, len(self.internal_node_list) - 1)]
    ...
    def local_path(self, volume):
        ...
        return "sheepdog:%(addr)s:%(port)s:%(name)s" % {
            'addr': self.get_addr(type='internal'),
            'port': self.port,
            'name': volume['name']}
    ...
    def update_node_list(self):
        ...
        try:
            (_stdout, _stderr) = self._run_dog('node', 'list', '-r', '-e')
        except SheepdogCmdError as e:
            with excutils.save_and_reraise_exception():
                LOG.error(_LE('Failed to get external node list. %s'), e)
        node_list = []
        stdout = _stdout.strip('\n')
        for line in stdout.split('\n'):
            node_list.append(line.split()[1].split(':')[0])
        self.external_node_list = node_list

        try:
            (_stdout, _stderr) = self._run_dog('node', 'list', '-r', '-i')
        except SheepdogCmdError as e:
            with excutils.save_and_reraise_exception():
                LOG.error(_LE('Failed to get internal node list. %s'), e)
        node_list = []
        stdout = _stdout.strip('\n')
        for line in stdout.split('\n'):
            node_list.append(line.split()[1].split(':')[0])
        self.internal_node_list = node_list
...
class SheepdogDriver(driver.VolumeDriver):
    ...
    def initialize_connection(self, volume, connector):
        target_host = self.client.get_addr(type='external')
        LOG.info("volume %s is exported in node %s." % (volume.name, target_host))
        return {
    ...
    def backup_volume(self, context, backup, backup_service):
        ...
        return backup_service.backup(backup, "sheepdog")
    ...
    def restore_backup(self, context, backup, volume, backup_service):
        ...
        backup_service.restore(backup, volume['id'], "sheepdog")
